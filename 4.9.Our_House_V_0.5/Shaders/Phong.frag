...
uniform float screen_width_num = 3.0f;
uniform float screen_height_num = 2.0f;

uniform bool ina_effect= false;
uniform bool ina_light_effect= false;
uniform bool ina_timer =false;

uniform bool blind_effect = false;
uniform float blind_width = 90.0f;

uniform bool cartoon_effect = false;
uniform float cartoon_levels = 3.0f;
	...
	for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
		...
		if (u_light[i].position.w != zero_f) { // point light source
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if(ina_light_effect && ina_timer)
					tmp_float = zero_f;

				else if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if(blind_effect){
						tmp_float = pow(tmp_float,u_light[i].spot_exponent)*cos(blind_width*acos(tmp_float));
					}
		...
		if (local_scale_factor > zero_f) {				
			...
			if (tmp_float > zero_f) {
				if(cartoon_effect){
					local_color_sum += u_light[i].diffuse_color*u_material.diffuse_color*floor(tmp_float*cartoon_levels)/cartoon_levels;
				}
				else
...
void main(void) {   
	...
	if(screen_effect){
		float x_mod,y_mod;
		x_mod = mod(v_position_sc.x*screen_width_num,1.0f);
		y_mod = mod(v_position_sc.y*screen_height_num,1.0f);
		if((x_mod>screen_width)&&(x_mod<1-screen_width)&&(y_mod>screen_width)&&(y_mod<1-screen_width))
			discard;
	}
	if(ina_effect){
		float x_mod,y_mod;
